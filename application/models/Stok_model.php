...
class Stok_model extends CI_Model {
    ...
    function clean()
    {
		...
		if ($clean) {
				$app = array(
					'nm_app' 		=> 'Chasier',
					'nama_toko' 	=> 'Toko Online',
					'alamat_toko' 	=> 'Jl. Parakanysag No. 14 - Kota Tasikmalaya',
					'home_txt' 		=> '<strong>Chasier App (Point of Sales)</strong> adalah aplikasi sederhana yang dapat digunakan untuk mengontrol keluar masuk barang serta dapat melakukan transaksi penjualan secara langsung.',
					'footer_txt' 	=> ''
				);
    ...
    function chart(){
        $query = $this->db->query("SELECT id_brg,SUM(jml_jual) AS jml_jual FROM penjualan GROUP BY id_brg");
